#!/usr/bin/env fish
# FuzzySeek shell integration for Fish
# Source this file or eval (fuzzyseek --shell-integration fish)

# Guard against double-sourcing
if set -q __FUZZYSEEK_FISH_LOADED
    exit 0
end
set -g __FUZZYSEEK_FISH_LOADED 1

# --- Configuration ---
set -q FUZZYSEEK_CMD; or set -g FUZZYSEEK_CMD fuzzyseek
set -q FUZZYSEEK_DEFAULT_OPTS; or set -g FUZZYSEEK_DEFAULT_OPTS ""

# --- Ctrl+R: History search ---
function __fuzzyseek_history
    set -l query (commandline)
    set -l output (
        builtin history | command $FUZZYSEEK_CMD $FUZZYSEEK_DEFAULT_OPTS --query "$query" 2>/dev/tty
    )
    if test $status -eq 0; and test -n "$output"
        commandline -r "$output"
    end
    commandline -f repaint
end

# --- Ctrl+T: File search ---
function __fuzzyseek_file
    set -l output (
        find . -path '*/.*' -prune -o -type f -print -o -type l -print 2>/dev/null |
        sed 's|^\./||' |
        command $FUZZYSEEK_CMD $FUZZYSEEK_DEFAULT_OPTS --multi 2>/dev/tty
    )
    if test $status -eq 0; and test -n "$output"
        commandline -it -- (string join ' ' -- $output)
    end
    commandline -f repaint
end

# --- Alt+C: cd to directory ---
function __fuzzyseek_cd
    set -l output (
        find . -path '*/.*' -prune -o -type d -print 2>/dev/null |
        sed 's|^\./||' |
        command $FUZZYSEEK_CMD $FUZZYSEEK_DEFAULT_OPTS 2>/dev/tty
    )
    if test -n "$output"
        cd -- "$output"
    end
    commandline -f repaint
end

# --- Bind keys ---
bind \cr __fuzzyseek_history
bind \ct __fuzzyseek_file
bind \ec __fuzzyseek_cd

# Also bind in insert mode if using vi mode
if bind -M insert >/dev/null 2>&1
    bind -M insert \cr __fuzzyseek_history
    bind -M insert \ct __fuzzyseek_file
    bind -M insert \ec __fuzzyseek_cd
end
